...
Стикер, контакт и аудио должны быть единственным вложением в сообщении (так требует API) — библиотека
...
- `Recover()` - восстановление после паник, с ответом пользователю
...
```go
b.Handle("/start", handler, middleware.Recover())
```
