...
`api.go` communicates with `https://platform-api.max.ru`. The `Bearer ` prefix is applied to the token once in `NewBot()` so all code paths use it automatically. `Raw()` is the generic request method. File uploads go through a two-step `GetUploadURL()` → `UploadFile()` flow.
